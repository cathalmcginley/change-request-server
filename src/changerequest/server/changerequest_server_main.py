...
from changerequest.server.changerequest_db import ChangeRequestDatabase
...
def changerequest_server_cli_main():
    cli_argument_parser = argparse.ArgumentParser(prog='changerequest-server', description='start ChangeRequest Server local web service', allow_abbrev=True, add_help=True)
    ...
    archetypes = ChangeRequestArchetypes()
    archetypes.load_releases("/home/cathal/change-requests/releases.yaml")
    app.config["archetypes"] = archetypes
    app.debug = False

    database = ChangeRequestDatabase("/home/cathal/change-requests")
